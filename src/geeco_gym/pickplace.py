"""Defines gym environments for pick-and-place tasks."""
...
MODEL_XML_PATH = {
    'pad1-cube1' : os.path.abspath(
        os.path.join(GEECO_ROOT, 'assets', 'gym', 'envs', 'geeco-pad1-cube1.xml')),
    'pad2-cube1' : os.path.abspath(
        os.path.join(GEECO_ROOT, 'assets', 'gym', 'envs', 'geeco-pad2-cube1.xml')),
    'pad1-cube2' : os.path.abspath(
        os.path.join(GEECO_ROOT, 'assets', 'gym', 'envs', 'geeco-pad1-cube2.xml')),
    'pad2-cube2' : os.path.abspath(
        os.path.join(GEECO_ROOT, 'assets', 'gym', 'envs', 'geeco-pad2-cube2.xml')),
    'pad2-cube2-clutter4' : os.path.abspath(
        os.path.join(GEECO_ROOT, 'assets', 'gym', 'envs', 'geeco-pad2-cube2-clutter4.xml')),
    'pad2-cube2-clutter12' : os.path.abspath(
        os.path.join(GEECO_ROOT, 'assets', 'gym', 'envs', 'geeco-pad2-cube2-clutter12.xml')),
    # generalization tasks
    'ball-cup' : os.path.abspath(
        os.path.join(GEECO_ROOT, 'assets', 'gym', 'envs', 'geeco-ball-cup.xml')),
    'bridge-pad' : os.path.abspath(
        os.path.join(GEECO_ROOT, 'assets', 'gym', 'envs', 'geeco-bridge-pad.xml')),
    'diamond-pad' : os.path.abspath(
        os.path.join(GEECO_ROOT, 'assets', 'gym', 'envs', 'geeco-diamond-pad.xml')),
    'nut-cone' : os.path.abspath(
        os.path.join(GEECO_ROOT, 'assets', 'gym', 'envs', 'geeco-nut-cone.xml')),
}
...
ROBOT_XPOS0 = np.array([1.3419, 0.7491, 0.555])
...
class PickAndPlaceEnv(fetch_env.FetchEnv, gym_utils.EzPickle):
  """Adapted gym.envs.robotics:FetchEnv for pick & place."""

  def __init__(
      self,
      work_dir,
      shapes='pad1-cube1', reward_type='sparse',
      rendering_mode='viewer', frame_res=(256, 256)):
    """
    :param shapes: pad1-cube1 | pad2-cube1 | pad1-cube2 | pad2-cube2 | \
    ...
    self.task_goal = None  # list of names of goal objects in pick&place task
    self.task_object = None  # list of names of manipulated objects in pick&place task
    self.target_frame = None  # list of RGB(D) target frames depicting desired outcome
    # --- rendering
  ...
  def _reset_replay_buffer(self):
    ...
    Called by PickAndPlaceEnv.reset().
    Called by PickAndPlaceEnv() constructor.
    """
    self.step_buffer = []
    self.time_elapsed = []
    self.rgb_buffer = []
  ...
  def _record_data_frame(self, action):
    ...
    goal_jnt_name = '%s:joint' % (str(self.task_goal[0]), )
    obj_jnt_name = '%s:joint' % (str(self.task_object[0]), )
  ...
  def start_tfrecorder(self, record_name):
    ...
    sequence_context['task_goal'] = str(self.task_goal[0])
    sequence_context['task_object'] = str(self.task_object[0])
  ...
  def _reset_sim(self):
    ...
    color_map = {  # geom_name -> RGBA
        ...
        'object2' : np.array([1, 0, 1, 1]),  # magenta cube
        'goal0' : np.array([0, 0, 1, 1]),  # blue goal
        'goal1' : np.array([0, 1, 0, 1]),  # green goal
        'goal2' : np.array([0, 1, 1, 1]),  # cyan goal
        'clutter0' : np.array([1, 0, 0, 1]),  # red clutter
        'clutter1' : np.array([1, 1, 0, 1]),  # yellow clutter
        'clutter2' : np.array([0, 0, 1, 1]),  # blue clutter
        'clutter3' : np.array([0, 1, 0, 1]),  # green clutter
        'clutter4' : np.array([1, 0, 0, 1]),  # red clutter
        'clutter5' : np.array([1, 1, 0, 1]),  # yellow clutter
        'clutter6' : np.array([0, 0, 1, 1]),  # blue clutter
        'clutter7' : np.array([0, 1, 0, 1]),  # green clutter
        'clutter8' : np.array([1, 0, 0, 1]),  # red clutter
        'clutter9' : np.array([1, 1, 0, 1]),  # yellow clutter
        'clutter10' : np.array([0, 0, 1, 1]),  # blue clutter
        'clutter11' : np.array([0, 1, 0, 1]),  # green clutter
    }
    color_map_mono = {  # geom_name -> RGBA; Which blue are you?
        'object0' : np.array([0, 0, 0.75, 1]),  # light-blue cube
        'object1' : np.array([0, 0, 0.75, 1]),  # light-blue cube
        'object2' : np.array([0, 0, 0.75, 1]),  # light-blue cube
        'goal0' : np.array([0, 0, 0.75, 1]),  # dark-blue goal
        'goal1' : np.array([0, 0, 0.75, 1]),  # dark-blue goal
        'goal2' : np.array([0, 0, 0.75, 1]),  # dark-blue goal
        'clutter0' : np.array([0, 0, 0.75, 1]),  # sky-blue clutter
        'clutter1' : np.array([0, 0, 0.75, 1]),  # sky-blue clutter
        'clutter2' : np.array([0, 0, 0.75, 1]),  # sky-blue clutter
        'clutter3' : np.array([0, 0, 0.75, 1]),  # sky-blue clutter
    }
    color_pool = [
    ...
    obj_names = [n for n in self.sim.model.site_names \
      if n.startswith('object') or n.startswith('goal') or n.startswith('clutter')]
    goal_names = [n for n in obj_names if n.startswith('goal')]
    cube_names = [n for n in obj_names if n.startswith('object')]
    clutter_names = [n for n in obj_names if n.startswith('clutter')]

    # --- re-color objects
    for obj_name in obj_names:
      if obj_name in color_map:
        if self.shapes == 'bin2-cube2-clutter-mono':
          color = color_map_mono[obj_name]
        else:
          color = color_map[obj_name]
      else:  # object not to be modified
        rnd_color_idx = np.random.randint(0, len(color_pool))
        color = color_pool[rnd_color_idx]
    ...
    if len(self.reset_queue) > 0:  # take reset state from queue
      ...
      if 'target' in reset_state:
        ...
        print("Registered %d target frames." % (len(self.target_frame), ))
        self.num_subgoals = np.min([len(self.task_object), len(self.task_goal), len(self.target_frame)])
      else:
        self.num_subgoals = np.min([len(self.task_object), len(self.task_goal)])
      # define task member
    else:  # randomize init state & task
      shapes2dims = {  # TODO: compute workspace extensions dynamically!
          'pad1-cube1' : {'minmax_x' : (1.075, 1.425), 'minmax_y' : (0.350, 1.150), 'tiling_xy' : (6, 8)},
          'pad1-cube2' : {'minmax_x' : (1.075, 1.425), 'minmax_y' : (0.350, 1.150), 'tiling_xy' : (6, 8)},
          'pad2-cube1' : {'minmax_x' : (1.075, 1.425), 'minmax_y' : (0.350, 1.150), 'tiling_xy' : (4, 7)},
          'pad2-cube2' : {'minmax_x' : (1.075, 1.425), 'minmax_y' : (0.350, 1.150), 'tiling_xy' : (4, 7)},
          'pad2-cube2-clutter4' : {'minmax_x' : (1.075, 1.425), 'minmax_y' : (0.350, 1.150), 'tiling_xy' : (4, 7)},
          'pad2-cube2-clutter12' : {'minmax_x' : (1.075, 1.425), 'minmax_y' : (0.350, 1.150), 'tiling_xy' : (4, 7)},
          # generalization scenarios
          'ball-cup' : {'minmax_x' : (1.075, 1.425), 'minmax_y' : (0.350, 1.150), 'tiling_xy' : (3, 6)},
          'bridge-pad' : {'minmax_x' : (1.075, 1.425), 'minmax_y' : (0.350, 1.150), 'tiling_xy' : (3, 6)},
          'diamond-pad' : {'minmax_x' : (1.075, 1.425), 'minmax_y' : (0.350, 1.150), 'tiling_xy' : (3, 6)},
          'nut-cone' : {'minmax_x' : (1.075, 1.425), 'minmax_y' : (0.350, 1.150), 'tiling_xy' : (3, 6)},
      }
      ...
      for obj_idx, (obj_name, spawn_pos) in enumerate(zip(obj_names, spawn_points)):
        ...
        object_qpos[2] = 0.27 + 0.037  # TODO: get table & object height dynamically!
        object_qpos[3:] = object_quat
        self.sim.data.set_joint_qpos(jnt_name, object_qpos)
      # randomize intial robot state
      robot_xpos0 = ROBOT_XPOS0 \
        + np.array(sample_point_within_sphere(radius=0.03))
      ...
      # define task member
      self.num_subgoals = len(self.task_goal)
  ...
  def _sample_goal(self):
    ...
    Goal is target position of object for pick&place.
    """
    if self.has_object:
